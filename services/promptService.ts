...
const getNotepadInstruction = (notepadContent: string): string => {
  return `\n\n--- notepad内容 ---\n${notepadContent || '当前notepad为空。'}\n--- notepad结束 ---\n\n你的任务分为两部分：\n1.  根据上述notepad和当前对话及你的身份，提供你的观点和回应。\n2.  在你完成回应后，你【必须】输出一个更新后的完整notepad。**将所有notepad内容包裹在 <notepad>...</notepad> 标签内**。请在notepad中添加新的经讨论核实确认后的观察与结论，并将过时包括错误的信息用 [过时] 标记出来。如果notepad内容没有变化，请在标签内重复原有内容。`;
};
